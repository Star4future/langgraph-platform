    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6">
      ...
      <section className="mt-4 space-y-3 text-sm leading-6 text-ink-dim">
        <p>
          ...
          — a multi-agent customer-workflow engine with an industry-agnostic
          core and pluggable verticals — plus this console, a Next.js front end
          that renders the engine&apos;s event stream as it arrives. The chat,
          the pipeline panel and the timeline are all driven by one typed SSE
          protocol whose schema lives in a single file shared by the CLI, the
          test suite and this UI.
        </p>
        <p>
          <span className="text-ink">It runs in mock mode on purpose.</span>{" "}
          No LLM key is configured here, so the engine answers from a
          deterministic keyword-matched mock. That keeps the demo free, fast
          and reproducible — and it means what you are watching is the{" "}
          <em>pipeline</em>: triage, tool dispatch, supervisor scoring, the
          retry loop, human escalation on a checkpoint. Streaming, however, is
          real — open the network tab and you&apos;ll find one
          text/event-stream response per run, not a choreographed animation.
        </p>
        <p>
          ...
          of those gaps are closed and which stay open by design.
        </p>
        <p>
          <span className="text-ink">About that expert review:</span> v1 was
          put through a commissioned adversarial review, published verbatim —
          including a 2/5 for production readiness and a 2/5 for eval rigour —
          with a resolution log recording what each finding became. Publishing
          the scores next to the fixes is the point: the demo badge says mock,
          the evals page says smoke test, and the review says where the edges
          are.
        </p>
      </section>
